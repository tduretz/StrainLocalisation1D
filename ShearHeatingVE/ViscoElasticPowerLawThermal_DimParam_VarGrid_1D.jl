...
@views function ViscoElastic_CrankNicolson(;θM=1.0, θT=1.0, nres=1, adapt_dt=false, viz=false)
    ...
    c_fact   = 1.8 # Damping
    rel      = 0.5
    bdf      = 0   # if 0 then NO BDF is used (CN or BE depending on θM, θT) 
    debug    = false
    nitermax = 3e5
    nresmax  = 32
    spY      = 3600*24*365.25
    ...
    if mesh_refinement
        ...
        yc0    = LinRange(y.min+Δy0/2, y.max-Δy0/2, ncy)
    ...
    @info tvec[imax]
    @info twall
    return vmax, tvec[imax], minimum(log10.(Δtvec)), maximum(log10.(Δtvec)), twall 
end
