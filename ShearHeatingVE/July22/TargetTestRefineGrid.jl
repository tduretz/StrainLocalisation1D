...
@views function ViscoElastic_CrankNicolson(name;θM=1.0, θT=1.0, nresy=1, nrest=1, adapt_dy=false, adapt_dt=false, viz=false, noisy=false)
    ...
    if adapt_dy
        ...
        # error()
        yv, yce, Δy_c = makeExpoGrid(L, ncy, L/3200, y.min)
        yc            = yce[2:end-1]
        Δy_v          = diff(yce)
        ncy_eq        = Int64(ceil((y.max - y.min) / minimum(Δy_c)))
    else
    ...
    @time twall = @elapsed while t < t_end && it<nitmax
        ...
        for iter=1:nitermax
            ...
            if iter == nitermax && !stopping
                @show norm(Rx)/ncy_eq, norm(Rx)/ncy_eq/nRx0, nRx0
                @show norm(RT)/ncy_eq, norm(RT)/ncy_eq/nRT0, nRT0
                error("Convergence failed: "*name)
            end
        end
        Δtvec[it]       = Δt
        tvec[it]        = t
        ...
        if (mod(it, vstep)==0 || it==Nt || it==nitmax) && viz
            ...
            #p1 = plot!(tvec[1:vstep:it] ./ spY, τvec_ana[1:vstep:it] ./ 1e6, label="ref", ls=:dash )
            p2 = plot(T[2:end-1] .* sc.T, yc .* sc.L ./ 1e3, label="T")
            # p3 = plot(log10.(ηv), yv, label="η")
            p3 = scatter(tvec[1:vstep:it] .* sc.t ./ spY, probes.vmax[1:vstep:it], label="Vmax")
            p4 = plot(Vx[2:end-1] .* sc.L/sc.t, yc .* sc.L ./ 1e3, label="Vx")
            p5 = plot(legend=:bottomright)
            p5 = scatter!(tvec[1:vstep:it] .* sc.t ./ spY, log10.(err.T[1:vstep:it]), label="err T")
            p5 = scatter!(tvec[1:vstep:it] .* sc.t ./ spY, log10.(err.V[1:vstep:it]), label="err V")
            p6 = plot(yc, log10.(Δy_c./Δy_c[1]), label = "Δy")
            display(plot(p1,p2,p3,p4,p5,p6, layout=(3, 2)))
        end
    if mod(it, 1000) == 0
        jldsave(name*".jld2"; evo=probes, τ=τvec, t=tvec, Δt=Δtvec, Δy=Δy_v)
    end
    end 
...
function checkGridLength(n, d0, f)
    ...
    elseif f == 1.0
        ...
        return d0 * (f^(n) - 1.0) / (f - 1.0)
    end
end

function findGrowthFactor(L, n, d0)
    # use bisection to find best growth factor
    a = 1.0
    b = 2.0
    for i = 1 : 20
        c = (a+b) / 2.0
